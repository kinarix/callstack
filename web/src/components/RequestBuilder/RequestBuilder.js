...
function getActiveEnvKey(projectId) {
    return projectId ? `callstack.activeEnv.${projectId}` : null;
}
export function RequestBuilder({ request, showExpandBtn, onExpand }) {
    ...
    const saveToDb = useCallback((id, changes) => {
        ...
        saveTimer.current = setTimeout(() => {
            ...
            if (changes.method !== undefined)
                fields.method = changes.method;
            if (changes.url !== undefined)
                fields.url = changes.url;
            if (changes.params !== undefined)
                fields.params = JSON.stringify(changes.params);
            if (changes.headers !== undefined)
                fields.headers = JSON.stringify(changes.headers);
            if (changes.body !== undefined)
                fields.body = changes.body;
            if (changes.files !== undefined)
                fields.attachments = JSON.stringify(changes.files);
    ...
    const handleRequestChange = (changes) => {
        if (request) {
            setBodyError(null);
            dispatch({ type: 'UPDATE_REQUEST', payload: { ...request, ...changes } });
            saveToDb(request.id, changes);
    ...
    const handleEnvSelect = (env) => {
        const key = getActiveEnvKey(request?.project_id ?? null);
        if (key)
            localStorage.setItem(key, String(env.id));
        setActiveEnvId(env.id);
    };
    const projectEnvironments = state.environments.filter((e) => e.project_id === (request?.project_id ?? -1));
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            saveResponse(request.id, result.status, result.statusText, result.headers, result.body, result.timeMs, result.size, sentAt).catch(console.error);
            const log = {
                id: ++logIdCounter,
                timestamp: sentAt,
                method: request.method,
    ...
    return (_jsxs("div", { className: styles.builder, children: [_jsx(UrlBar, { request: request, isLoading: state.isLoading, urlError: urlError, showExpandBtn: showExpandBtn, onExpand: onExpand, onMethodChange: handleMethodChange, onUrlChange: handleUrlChange, onNameChange: (name) => handleRequestChange({ name }), onSend: handleSend, followRedirects: followRedirects, onFollowRedirectsChange: setFollowRedirects, environments: projectEnvironments, activeEnvId: activeEnvId, onEnvSelect: handleEnvSelect }), urlError && (_jsxs("div", { className: styles.bodyError, children: [_jsx("span", { className: styles.bodyErrorIcon, children: "\u26A0" }), urlError.message] })), bodyError && (_jsxs("div", { className: styles.bodyError, children: [_jsx("span", { className: styles.bodyErrorIcon, children: "\u26A0" }), bodyError] })), _jsxs("div", { className: styles.panes, style: { gridTemplateColumns: `${splitPct}fr 4px ${100 - splitPct}fr` }, children: [_jsx("div", { className: styles.requestPane, children: _jsx(TabPanel, { request: request, onRequestChange: handleRequestChange, files: files, onFilesChange: handleFilesChange }) }), _jsx("div", { className: styles.splitHandle, onMouseDown: startPanelResize }), _jsx("div", { className: styles.responsePane, children: _jsx(ResponseViewer, { response: state.currentResponse, requestName: request?.name, onClear: () => dispatch({ type: 'SET_RESPONSE', payload: null }) }) })] })] }));
}
